...
async function main() {
  ...
  // 2) Read & filter out blanks, comments, license lines
  ...
  for (const line of corpusLines) {
    ...
    const locClean = locRaw.replace(/[()]/g, '')
    const [sura, verse, wordIdx] = locClean.split(':')
  ...
  // 5) Merge QAC into Quran tokens
  const merged = []
  for (const verseLine of quranLines) {
    const [sura, verse, text] = verseLine.split('|')
    ...
    tokens.forEach((token, i) => {
      ...
      const key   = `${sura}|${verse}|${idx}|${bwKey}`
      ...
      if (hits.length) {
        ...
        console.warn(
          `⚠️ Missing QAC entry for ${sura}:${verse} token #${idx} (“${token}”)`
        )
        merged.push({
          sura:      Number(sura),
          verse:     Number(verse),
